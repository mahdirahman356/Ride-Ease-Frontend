...
const Requests = () => {
    ...
    const { data: rideRequeste } = useGetRideRequestsQuery(undefined)
                ...
                <TableBody>
                    {rideRequeste?.data?.map((item: IRide) => (
                        <TableRow key={item._id}>
                            ...
                            <TableCell >
                                <Button onClick={() => handleStatus(item._id, "ACCEPTED")} className="mr-4">Accept</Button>
                                <Button onClick={() => handleStatus(item._id, "REJECTED")}>Reject</Button>
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
